...
bot.command('globalstories', async (ctx) => {
  ...
  let controlsMessageId: number | undefined;
  try {
    const reply = await ctx.reply(t(locale, 'global.label'), {
      ...extraOptions,
      reply_markup: {
        inline_keyboard: [
          [
            {
              text: `${t(locale, 'pagination.next')} ${GLOBAL_STORIES_PAGE_SIZE}`,
              callback_data: `${GLOBAL_STORIES_CALLBACK_PREFIX}${GLOBAL_STORIES_PAGE_SIZE}`,
            },
          ],
        ],
      },
    });
    controlsMessageId = reply.message_id;
  } catch (error) {
    console.error('Failed to send global stories controls:', error);
  }
  ...
  const task: UserInfo = {
    ...
    offset: 0,
    globalStoriesMessageId: controlsMessageId,
  };
  handleNewTask(task);
});
