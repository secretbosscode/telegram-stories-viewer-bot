...
export async function handleNewTask(user: UserInfo) {
  ...
  try {
    ...
    // Poke the queue processor to start immediately if it's idle.
...
export async function processQueue() {
  if (isProcessing) {
    return; // A loop is already running, exit to prevent concurrency issues.
  }

  const job: DownloadQueueItem | null = await getNextQueueItemFx();

  if (!job) {
    // No jobs in the queue, the loop can stop.
    return;
  }
  ...
  try {
    ...
    // FINAL FIX: Use a finally block to guarantee the lock is released.
    isProcessing = false;
    // Clean up old jobs from the queue.
    await cleanupQueueFx();
    // Immediately check for the next job to continue the loop.
    setImmediate(processQueue);
  }
}
